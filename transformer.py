...
class ScaledDotProductAttention:
    ...
    def __call__(self, q, k, v, mask):  # mask_k or mask_qk
        # d_k = tf.cast(tf.shape(k)[-1], type='float32')
        # d_k = Lambda(lambda x: tf.cast(tf.shape(x)[-1], 'float32'))(k)
        # temper = tf.sqrt(d_k)
        # temper = tf.sqrt(tf.cast(tf.shape(k)[-1], dtype='float32'))
        scores = Lambda(lambda x: K.batch_dot(x[0], x[1], axes=[2, 2]))([q, k])
        scaled_scores = Lambda(
            lambda x: x[0] / tf.sqrt(tf.cast(tf.shape(x[1])[-1], "float32"))
        )([scores, k])
        attn = Lambda(lambda x: x[0] / tf.sqrt(tf.cast(tf.shape(x[1])[-1], "float32")))(
            [scores, k]
        )
        # attn = Lambda(lambda x:K.batch_dot(x[0],x[1],axes=[2,2])/x[2])([q, k, temper])  # shape=(batch, q, k)
        if mask is not None:
            mmask = Lambda(lambda x: (-1e9) * (1.0 - K.cast(x, "float32")))(mask)
            scaled_scores = Add()([scaled_scores, mmask])
            # attn = Add()([attn, mmasddk])
        ...
        # attn = Activation("softmax")(scaled_scores)
        attn = Lambda(
            lambda x: tf.cond(
...
class DecoderLayer:
    ...
    def __call__(
        self, dec_input, enc_output, self_mask=None, enc_mask=None, dec_last_state=None
    ):
        if dec_last_state is None:
            dec_last_state = dec_input
        output, slf_attn = self.self_att_layer(
            dec_input, dec_last_state, dec_last_state, mask=self_mask
        )
...
class SelfAttention:
    def __init__(self, d_model, d_inner_hid, n_head, layers=6, dropout=0.1):
        self.layers = [
            EncoderLayer(d_model, d_inner_hid, n_head, dropout) for _ in range(layers)
        ]
...
class Decoder:
    def __init__(self, d_model, d_inner_hid, n_head, layers=6, dropout=0.1):
        self.layers = [
            DecoderLayer(d_model, d_inner_hid, n_head, dropout) for _ in range(layers)
        ]

    def __call__(
        self, tgt_emb, tgt_seq, src_seq, enc_output, return_att=False, active_layers=999
    ):
        x = tgt_emb
        self_pad_mask = Lambda(lambda x: GetPadMask(x, x))(tgt_seq)
        self_sub_mask = Lambda(GetSubMask)(tgt_seq)
        self_mask = Lambda(lambda x: K.minimum(x[0], x[1]))(
            [self_pad_mask, self_sub_mask]
        )
...
class DecoderPerStep(Layer):
    ...
    def call(self, inputs):
        ...
        col_mask = K.cast(
            K.equal(K.cumsum(K.ones_like(tgt_embs[0], dtype="int32"), axis=1), llen),
            dtype="float32",
        )
        rs = [x]
        for i, dec_layer in enumerate(self.layers):
            tgt_emb = tgt_embs[i] + x * col_mask
            print(f"Layer {i}: x shape = {x.shape}, tgt_emb shape = {tgt_emb.shape}")
            x, _, _ = dec_layer(
                x, enc_output, enc_mask=enc_mask, dec_last_state=tgt_emb
            )
            rs.append(x)
        return rs
...
class ReadoutDecoderCell(Layer):
    ...
    def call(self, inputs, states, constants, training=None):
        (tgt_curr_input, tgt_pos_input, dec_mask), dec_output = (
            states[:3],
            list(states[3:]),
        )
        ...
        col_mask = K.cast(
            K.equal(K.cumsum(K.ones_like(dec_mask), axis=1), time), dtype="int32"
        )
        ...
        for i, dec_layer in enumerate(self.decoder.layers):
            ...
            x, _, _ = dec_layer(
                x, enc_output, dec_mask, enc_mask, dec_last_state=dec_last_state
            )
...
def decode_batch_greedy(
    src_seq, encode_model, decode_model, start_mark, end_mark, max_len=128
):
    ...
    for i in range(max_len - 1):
        outputs = [
            x.numpy()
            for x in decode_model([target_one, src_seq, enc_ret] + dec_outputs)
        ]
        ...
        dec_outputs = [
            np.concatenate([x, np.zeros_like(new_out)], axis=1) for x in dec_outputs
        ]

        sampled_indexes = np.argmax(output[:, 0, :], axis=-1)
        print(f"Predicted token ID: {sampled_indexes[0]}, <PAD> ID: {end_mark}")
    ...
    for i in range(max_len - 1):
        ...
        for ii in range(N):
            ...
            for k, wprobs in zip(range(lastks[ii]), output[base:, :]):
                prev = base + k
                if (
                    len(decoded_indexes[prev]) > 0
                    and decoded_indexes[prev][-1] == end_mark
                ):
                    continue
...
class Transformer:
    ...
    def compile(self, optimizer="adam", active_layers=999):
        ...
        def get_accu(y_true, y_p):
            ...
            corr = K.cast(
                K.equal(
                    K.cast(y_true, "int32"), K.cast(K.argmax(y_pred, axis=-1), "int32")
                ),
                "float32",
            )
    ...
    def make_readout_decode_model(self, max_output_len=32):
        ...
        cell = ReadoutDecoderCell(
            self.o_word_emb, self.pos_emb, self.decoder, self.target_layer
        )
        final_output = InferRNN(cell, return_sequences=True)(
            rep_input,
            initial_state=[
                tgt_start_input,
    ...
    def decode_sequence_readout_x(self, X, batch_size=32, max_output_len=64):
        ...
        ret = self.readout_model.predict(
            [X, target_seq], batch_size=batch_size, verbose=1
        )
        return ret
    ...
    def generate_sentence(self, rets, delimiter=""):
        ...
        for x in rets:
            end_pos = min(
                [i for i, z in enumerate(x) if z == self.o_tokens.endid()] + [len(x)]
            )
    ...
    def make_fast_decode_model(self):
        ...
        dec_ret_inputs = [
            Input(shape=(None, self.d_model)) for _ in self.decoder.layers
        ]
        ...
        dec_outputs = self.decoder_pre_step(
            [tgt_one, src_seq_input, enc_ret_input] + dec_ret_inputs
        )
        final_output = self.target_layer(dec_outputs[-1])

        self.decode_model = Model(
            [tgt_one_input, src_seq_input, enc_ret_input] + dec_ret_inputs,
    ...
    def decode_sequence_fast(self, input_seqs, batch_size=32, delimiter="", verbose=0):
        ...
        decode_batch = lambda x: decode_batch_greedy(
            x, encode_model, decode_model, start_mark, end_mark, max_len
        )
    ...
    def beam_search(
        ...
        decode_batch = lambda x: decode_batch_beam_search(
            x, topk, encode_model, decode_model, start_mark, end_mark, max_len
        )
        ...
        for iter in rng:
            for i, x, y in decode_batch(src_seq[iter : iter + batch_size]):
                rets.setdefault(iter + i, []).append(
                    (x, y / np.power(len(x) + 1, length_penalty))
                )
        rets = {
            x: sorted(ys, key=lambda x: x[-1], reverse=True) for x, ys in rets.items()
        }
        rets = [rets[i] for i in range(len(rets))]

        rets = [
            [(delimiter.join(list(map(self.o_tokens.token, x))), y) for x, y in r]
            for r in rets
        ]
        if type(input_seqs[0]) is type("") and len(rets) == 1:
            rets = rets[0]
...
class QANet_ConvBlock:
    def __init__(self, dim, n_conv=2, kernel_size=7, dropout=0.1):
        self.convs = [
            SeparableConv1D(dim, kernel_size, activation="relu", padding="same")
...
class QANet_Block:
    def __init__(self, dim, n_head, n_conv, kernel_size, dropout=0.1, add_pos=True):
        self.conv = QANet_ConvBlock(
            dim, n_conv=n_conv, kernel_size=kernel_size, dropout=dropout
        )
