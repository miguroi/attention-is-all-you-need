    ...
    while True:
        for ss in ljqpy.LoadCSVg(fn):
            ...
            if len(Xs[0]) >= batch_size:
                X = pad_to_longest(Xs[0], itokens, max_len)
                Y = pad_to_longest(Xs[1], otokens, max_len)
                yield((X, Y), Y[:, 1:])

                # Both should have same length
                # decoder_input = Y[:, :-1]  # Remove last token
                # decoder_target = Y[:, 1:]  # Remove first token
                # print(f"Model expects output shape: {decoder_target.shape}")
                # print(f"Decoder input shape: {decoder_input.shape}")
                # Ensure same sequence length
                # min_len = min(decoder_input.shape[1], decoder_target.shape[1])
                # decoder_input = decoder_input[:, :min_len]
                # decoder_target = decoder_target[:, :min_len]
                # yield ((X, decoder_input), decoder_target)
                Xs = [[], []]
