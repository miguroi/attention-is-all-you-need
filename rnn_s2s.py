...
class RNNSeq2Seq:
    def __init__(self, i_tokens, o_tokens, latent_dim, layers=3):
        ...
        self.ppl = Lambda(lambda x: K.exp(x))(loss)
        ...
        decoder_model = Model([decoder_inputs] + decoder_states_inputs, [decoder_outputs] + list(decoder_states))
    ...
    def compile(self, optimizer):
        ...
        # Fix: Use proper loss function
        self.model.compile(optimizer=optimizer, loss=lambda y_true, y_pred: y_pred)
    ...
    def decode_sequence(self, input_seq, delimiter=''):
        ...
        # state_value = self.encoder_model.predict(input_mat, verbose=0)
        state_value = list(self.encoder_model.predict(input_mat, verbose=0))
        target_seq = np.zeros((1, 1))
        target_seq[0,0] = self.o_tokens.id('<S>')

        decoded_tokens = []
        while True:
            # output_tokens_and_h = self.decoder_model.predict([target_seq] + list(state_value), verbose=0)
            output_tokens_and_h = self.decoder_model.predict([target_seq] + state_value, verbose=0)
    ...
    class TestCallback(Callback):
        def on_epoch_end(self, epoch, logs = None):
            ...
            for test in [123, 12345, 34567]:
                ret = s2s.decode_sequence(list(str(test)))
                print(test, ret, hex(test))
            print('\n')

    #s2s.model.load_weights('model.h5')
    s2s.model.fit([X, Y], Y, batch_size=1024, epochs=10,  # Fix: Add Y as target
